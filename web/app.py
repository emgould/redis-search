    ...
    try:
        ...
        return JSONResponse(
            content={
                ...
                "maxmemory": stats.get("maxmemory", 0),
                "maxmemory_policy": stats.get("maxmemory_policy", "unknown"),
...
async def api_add_tmdb_to_redis(
    ...
    """Fetch a TMDB title, normalize it, and insert/update Redis media index."""
    global GENRE_MAPPING

    mc_type_normalized = mc_type.lower().strip()
    if mc_type_normalized not in {MCType.MOVIE.value, MCType.TV_SERIES.value}:
        return JSONResponse(
            status_code=400,
            content={"error": "mc_type must be one of: movie, tv"},
        )
    ...
    tmdb_media_type = MCType.MOVIE if mc_type_normalized == MCType.MOVIE.value else MCType.TV_SERIES
    service = TMDBService()
    redis = get_redis()
    ...
    try:
        ...
        mm_result: dict[str, Any] = {"attempted": False}
        mm_passed, mm_reason = passes_media_manager_filter(redis_doc)
        if mm_passed:
            try:
                mm_result["attempted"] = True
                mm_client = MediaManagerClient()
                try:
                    insert_resp = await mm_client.insert_docs([redis_doc])
                    mm_result["sent"] = True
                    mm_result["queued"] = insert_resp["queued"]
                    mm_result["skipped"] = insert_resp["skipped"]
                    mm_result["errors"] = insert_resp["errors"]
                finally:
                    await mm_client.close()
            except Exception as mm_exc:
                mm_result["sent"] = False
                mm_result["error"] = str(mm_exc)
        else:
            mm_result["sent"] = False
            mm_result["filter_reason"] = mm_reason
...
async def api_get_media_details(
    ...
    try:
        ...
        if force_refresh:
            try:
                existing_doc = await redis.json().get(key)  # type: ignore[assignment]
            except Exception:
                pass
...
# ---------------------------------------------------------------------------
# ETL VM log viewer – fetch docker logs from the ETL VM over gcloud SSH
# ---------------------------------------------------------------------------

_ETL_VM_NAME = os.getenv("ETL_VM_NAME", "etl-runner-vm")
_ETL_VM_ZONE = os.getenv("ETL_VM_ZONE", "us-central1-a")
_ETL_VM_CONTAINER = os.getenv("ETL_VM_CONTAINER", "etl-runner")


@app.get("/api/etl/vm/logs")
async def get_etl_vm_logs(
    since: str = Query("24h", description="Docker logs --since value, e.g. 24h, 12h, 2026-03-12"),
    tail: int | None = Query(None, description="Max lines to return", ge=1, le=10000),
    _: None = Depends(require_api_key),
) -> JSONResponse:
    """Fetch docker logs from the ETL VM via gcloud SSH + IAP tunnel."""

    def _fetch_logs() -> tuple[str, int]:
        cmd: list[str] = [
    ...
    try:
        output, returncode = await asyncio.to_thread(_fetch_logs)
        ...
        if returncode != 0:
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error": f"gcloud SSH exited {returncode}",
                    "logs": output[-4000:] if output else "",
                },
            )
        ...
        return JSONResponse(
            ...
            content={
                "success": False,
                "error": "Timed out connecting to ETL VM (45s). Is the VM running?",
            },
        )
        ...
        return JSONResponse(
            ...
            content={
                "success": False,
                "error": "gcloud CLI not found. This endpoint requires a local environment with gcloud installed.",
            },
        )
    except Exception as e:
        return JSONResponse(
