...
#   - TVDB_API_KEY (for TVDB API)
#   - TMDB_API_TOKEN (optional, for TMDB integration)
#
...
# Function to seed TVDB fixtures
seed_tvdb_fixtures() {
    print_header "Seeding TVDB Mock Data"

    # Check for required environment variables
    local missing_vars=()

    if ! check_env_var "TVDB_API_KEY"; then
        missing_vars+=("TVDB_API_KEY")
    fi
    ...
    if [ ${#missing_vars[@]} -gt 0 ]; then
        ...
        print_info "Get TVDB API key from: https://thetvdb.com/api-information"
        print_info "Then add to .env file or run:"
        echo "  export TVDB_API_KEY='your_key_here'"
        return 1
    fi

    print_success "All required environment variables are set"

    # Navigate to TVDB tests directory
    local tvdb_dir="$SCRIPT_DIR/tvdb/tests"
    
    if [ ! -d "$tvdb_dir" ]; then
        print_error "TVDB tests directory not found: $tvdb_dir"
        return 1
    fi
    ...
    if [ ! -f "fixtures/generate_mock_data.py" ]; then
        print_error "generate_mock_data.py not found in $tvdb_dir"
        return 1
    fi

    # Run generator for all fixture types
    print_info "Fetching real data from TVDB API..."
    ...
    if [ "$all_success" = true ]; then
        print_success "TVDB mock data seeded successfully"
        ...
        print_error "Failed to seed some TVDB mock data"
        return 1
    fi
}
...
show_env_status() {
    ...
    echo "TVDB:"
    if check_env_var "TVDB_API_KEY"; then
        print_success "TVDB_API_KEY is set"
    else
        print_error "TVDB_API_KEY is NOT set"
    fi

    echo ""
    echo "TMDB (optional):"
    if check_env_var "TMDB_API_TOKEN"; then
        print_success "TMDB_API_TOKEN is set"
    else
        print_warning "TMDB_API_TOKEN is NOT set (optional)"
    fi
}
...
main() {
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        print_info "Fixtures saved in services/tvdb/tests/fixtures/"
        echo ""
        print_info "Next steps:"
        echo "  1. Review generated fixtures"
        echo "  2. Run tests: cd .. && pytest services/tvdb/tests/ -v"
