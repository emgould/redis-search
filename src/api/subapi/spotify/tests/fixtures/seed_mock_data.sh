#!/bin/bash
#
# Seed Mock Data - Generate test fixtures from real Spotify API responses
#
...
# Function to seed Spotify fixtures
seed_spotify_fixtures() {
    print_header "Seeding Spotify Mock Data"
    ...
    # Navigate to Spotify tests directory
    local spotify_dir="$SCRIPT_DIR/spotify/tests"
    
    if [ ! -d "$spotify_dir" ]; then
        print_error "Spotify tests directory not found: $spotify_dir"
        return 1
    fi
    ...
    if [ ! -f "fixtures/generate_mock_data.py" ]; then
        print_error "generate_mock_data.py not found in $spotify_dir"
        return 1
    fi

    # Run generator for all fixture types
    print_info "Fetching real data from Spotify API..."
    ...
    if [ "$all_success" = true ]; then
        print_success "Spotify mock data seeded successfully"
        
        # Show what was generated
        print_info "Generated fixtures in:"
        for dir in fixtures/make_requests fixtures/search fixtures/models; do
            if [ -d "$dir" ]; then
        ...
        print_error "Failed to seed some Spotify mock data"
        return 1
    fi
}
...
main() {
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        print_info "Fixtures saved in api/spotify/tests/fixtures/"
        echo ""
        print_info "Next steps:"
        echo "  1. Review generated fixtures"
        echo "  2. Run tests: cd api/spotify && ./bin/run_tests.sh"
