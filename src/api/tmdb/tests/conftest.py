"""
Shared fixtures and utilities for TMDB service tests.

This conftest uses real API data that has been captured from TMDB.
...
def setup_tmdb_token():
    """Automatically set TMDB_READ_TOKEN for all tests if not already set.

    This ensures unit tests don't fail due to missing token.
    Integration tests should override this with real token from environment.
    """
    if not os.getenv("TMDB_READ_TOKEN"):
        os.environ["TMDB_READ_TOKEN"] = "test_tmdb_token_12345"
